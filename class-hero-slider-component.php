...
/**
 * Kreatus Hero Slider Component V3 (Optimized)
 *
 * DÜZELTMELER & İYİLEŞTİRMELER:
 * - ✅ Performans: Preview render optimize edildi (video yeniden yüklenmiyor)
 * - ✅ Sürükleme: Takılma sorunu çözüldü
 * - ✅ CSS: !important ile stil öncelikleri düzeltildi
 * - ✅ Animasyon: Motion effects, hover effects, idle animations eklendi
 * - ✅ Overlay: Gradient picker + opacity slider
 * - ✅ Particle: Preview'da gösteriliyor
 * - ✅ Image: Border radius desteği
 *
 * @package     Kreatus
...
class ClassHeroSliderComponent extends KreatusComponentBase {
    ...
    protected function init() {
        ...
        $this->name = '🚀 Hero Slider V3.1';
        $this->icon = '🎬';
        $this->description = 'Optimize edilmiş tam ekran düzenleme. Sorunsuz sürükleme + zengin animasyonlar!';
                        ...
                        <?php foreach ($slides as $index => $slide): ?>
                            ...
                            <div class="hsc-slide <?php echo $index === 0 ? 'active' : ''; ?>" 
                                ...
                                <?php if (!empty($slide['bg_overlay'])): ?>
                                    ...
                                    $overlay_opacity = floatval($slide['bg_overlay_opacity'] ?? 0.5);
                                    $overlay_style = '';
                                ...
                                <div class="hsc-slide-layers">
                                    ...
                                    foreach ($layers as $layer): 
                                        if (!empty($layer['hidden'])) continue;
                                        ...
                                        $hover_class = '';
                                        if (!empty($layer['hover_effect']) && $layer['hover_effect'] !== 'none') {
                                            $hover_class = 'hsc-hover-' . esc_attr($layer['hover_effect']);
                                        }
                                        
                                        $motion_class = '';
                                        if (!empty($layer['motion_effect']) && $layer['motion_effect'] !== 'none') {
                                            $motion_class = 'hsc-motion-' . esc_attr($layer['motion_effect']);
                                        }
                                    ?>
                                        <div id="<?php echo esc_attr($layer_id); ?>" 
                                             class="hsc-layer hsc-layer-<?php echo esc_attr($layer['type'] ?? 'text'); ?> <?php echo $hover_class; ?> <?php echo $motion_class; ?>"
                                            ...
                                            <?php elseif ($layer['type'] === 'image'): ?>
                                                <?php 
                                                $border_radius = !empty($layer['border_radius']) ? 'border-radius: ' . intval($layer['border_radius']) . 'px;' : '';
                                                $image_html = '<img src="' . esc_url($layer['image_url'] ?? '') . '" alt="' . esc_attr($layer['alt_text'] ?? 'layer') . '" style="' . $border_radius . '">';
    ...
    private function generate_layer_styles($layer) {
        $styles = '';
        $styles .= 'top: ' . esc_attr($layer['pos_y'] ?? '10') . 'px !important;';
        $styles .= 'left: ' . esc_attr($layer['pos_x'] ?? '10') . 'px !important;';
        
        if (!empty($layer['width']) && $layer['width'] !== 'auto') {
            $styles .= 'width: ' . esc_attr($layer['width']) . 'px !important;';
        }
        if (!empty($layer['height']) && $layer['height'] !== 'auto') {
            $styles .= 'height: ' . esc_attr($layer['height']) . 'px !important;';
        }
        
        $styles .= 'z-index: ' . esc_attr($layer['z_index'] ?? '10') . ' !important;';
        
        if ($layer['type'] === 'text' || $layer['type'] === 'button') {
            $styles .= 'font-size: ' . esc_attr($layer['font_size'] ?? '16') . 'px !important;';
            $styles .= 'color: ' . esc_attr($layer['color'] ?? '#000000') . ' !important;';
            $styles .= 'font-weight: ' . esc_attr($layer['font_weight'] ?? '400') . ' !important;';
        ...
        if ($layer['type'] === 'button') {
            $styles .= 'background-color: ' . esc_attr($layer['bg_color'] ?? '#0073aa') . ' !important;';
            $styles .= 'padding: ' . esc_attr($layer['padding'] ?? '10px 20px') . ' !important;';
            $styles .= 'border-radius: ' . esc_attr($layer['border_radius'] ?? '4') . 'px !important;';
        }
    ...
    private function generate_frontend_styles() {
        ...
        .hsc-hover-rotate:hover {
            transform: rotate(5deg) scale(1.05);
        }
        .hsc-hover-tilt {
            transition: transform 0.5s ease;
        }
        .hsc-hover-tilt:hover {
            transform: perspective(1000px) rotateX(10deg) rotateY(10deg);
        }
        
        /* MOTION EFFECTS */
        @keyframes hscFloating {
            0%, 100% { transform: translateY(0); }
            50% { transform: translateY(-15px); }
        }
        .hsc-motion-floating {
            animation: hscFloating 3s ease-in-out infinite;
        }
        ...
        .hsc-motion-swing {
            animation: hscSwing 3s ease-in-out infinite;
        ...
        .hsc-motion-bounce {
            animation: hscBounce 2s ease-in-out infinite;
        }
        ...
        .hsc-motion-wiggle {
            animation: hscWiggle 2s ease-in-out infinite;
        }
        
        @keyframes hscRotate360 {
            from { transform: rotate(0deg); }
            to { transform: rotate(360deg); }
        }
        .hsc-motion-rotate {
            animation: hscRotate360 20s linear infinite;
        }
    ...
    public function render_editor($content, $settings = array()) {
        ...
        $output .= '<h3>🚀 Hero Slider V3.1 (Optimized)</h3>';
        $output .= '<p style="color: #64748B; font-size: 0.9rem; margin: 0.5rem 0 1.5rem 0;">Performans optimize! Sorunsuz sürükleme + zengin animasyonlar!</p>';
            ...
            <div class="hsc-fullscreen-container">
                ...
                <div class="hsc-fullscreen-left">
                    <div class="hsc-fullscreen-header">
                        <div>
                            <h3 style="margin: 0;">✨ Hero Slider Editor V3.1</h3>
                            <small style="color: #64748B;">Optimize Edilmiş</small>
                        </div>
                            ...
                            <div id="hsc-slide-settings" class="hsc-section" style="display: none;">
                                ...
                                <div class="hsc-form-grid">
                                    ...
                                    <div class="kreatus-form-group hsc-grid-full">
                                        ...
                                        <span id="hsc-overlay-opacity-value" style="font-size: 0.9rem; color: #64748B;">0.5</span>
                                    </div>
                ...
                <div class="hsc-fullscreen-right">
                    ...
                    <div class="hsc-fullscreen-preview-info">
                        <small>💡 Katmanları TUTUP SÜRÜKLEYİN • Köşelerden boyutlandırın • Ok tuşları (Shift = 10px) • Del ile sil</small>
                    </div>
    ...
    private function render_editor_styles() {
        ...
        .kreatus-range::-webkit-slider-thumb {
            -webkit-appearance: none;
            appearance: none;
            width: 18px;
            height: 18px;
        ...
        .hsc-preview-snow {
            width: 5px;
            height: 5px;
            animation: hscPreviewSnowfall 10s linear infinite;
        }
        @keyframes hscPreviewSnowfall {
            0% { transform: translateY(-20px); opacity: 0; }
            10% { opacity: 0.8; }
            90% { opacity: 0.8; }
            100% { transform: translateY(calc(100% + 20px)) translateX(50px); opacity: 0; }
        }
        .hsc-preview-bubble {
            width: 20px;
            height: 20px;
            background: rgba(255,255,255,0.1);
            border: 1px solid rgba(255,255,255,0.2);
            animation: hscPreviewBubbleRise 15s ease-in-out infinite;
        }
        @keyframes hscPreviewBubbleRise {
            0% { transform: translateY(calc(100% + 20px)) scale(0); opacity: 0; }
            10% { opacity: 0.7; transform: scale(0.5); }
            90% { opacity: 0.7; }
...
private function render_editor_scripts() {
    ...
    (function($) {
        ...
        let hscPreviewUpdateTimeout = null;
        ...
        $(document).on('input', '#hsc-slide-overlay-opacity', function() {
            ...
            $('#hsc-overlay-opacity-value').text(value);
            
            if (hscCurrentSlideIndex !== -1) {
                const slide = hscSliderData.slides[hscCurrentSlideIndex];
                slide.bg_overlay_opacity = parseFloat(value);
                hscSchedulePreviewUpdate();
            }
        });
        
        // SAVE SLIDE SETTINGS (Optimize - sadece değişen field için preview update)
        ...
        window.hscAddLayer = function(type) {
            ...
            if (type === 'text') {
                ...
                newLayer.motion_effect = 'none';
                newLayer.border_radius = 0;
                newLayer.link_url = '';
        ...
        // SAVE LAYER SETTINGS (Optimize - sadece preview update değil layer update)
        $(document).on('change input', '#hsc-layer-settings-content input, #hsc-layer-settings-content select, #hsc-layer-settings-content textarea', function() {
            ...
            layer.motion_effect = $('#hsc-layer-motion-effect').val();
        ...
        // OPTIMIZE: Preview Update Scheduling (Video yeniden yüklenmeyi önle)
        function hscSchedulePreviewUpdate() {
            if (hscPreviewUpdateTimeout) {
                clearTimeout(hscPreviewUpdateTimeout);
            }
            hscPreviewUpdateTimeout = setTimeout(() => {
                hscUpdatePreviewLayers();
            }, 100);
        }
        
        // RENDER PREVIEW (Initial render - Video ile)
        function hscRenderPreview() {
            ...
            if (slide.bg_overlay) {
                ...
                if (slide.bg_overlay_type === 'gradient') {
                    overlayStyle = `background: linear-gradient(${slide.bg_overlay_gradient_direction}, ${slide.bg_overlay_gradient_start}, ${slide.bg_overlay_gradient_end}); opacity: ${opacity};`;
                } else {
                    overlayStyle = `background-color: ${slide.bg_overlay_color}; opacity: ${opacity};`;
                }
                overlayHTML = `<div class="hsc-preview-overlay" style="position: absolute; inset: 0; ${overlayStyle} z-index: 3; pointer-events: none;"></div>`;
            }
            ...
            if (slide.background_type === 'video' && slide.background_video) {
                videoHTML = `
                    <div id="hsc-preview-video-container" style="position: absolute; top: 50%; left: 50%; min-width: 100%; min-height: 100%; transform: translate(-50%, -50%); z-index: 2;">
                        <video id="hsc-preview-video" autoplay muted loop playsinline style="width: 100%; height: auto;">
            ...
            if (slide.particle_effect && slide.particle_effect !== 'none') {
                particlesHTML = '<div class="hsc-preview-particles" id="hsc-preview-particles"></div>';
            }
            
            $preview.html(`
                <div id="hsc-preview-bg" style="position: absolute; inset: 0; ${bgStyle} z-index: 1;"></div>
                ${videoHTML}
                ${overlayHTML}
                ${particlesHTML}
                <div id="hsc-preview-layers-container" style="position: relative; width: 100%; height: 100%; z-index: 5;"></div>
            `);
            
            // Render particles
            hscRenderPreviewParticles(slide.particle_effect);
        ...
        // UPDATE PREVIEW LAYERS ONLY (Video korunur)
        function hscUpdatePreviewLayers() {
            if (hscCurrentSlideIndex === -1) return;
            
            const slide = hscSliderData.slides[hscCurrentSlideIndex];
            const $container = $('#hsc-preview-layers-container');
            ...
            slide.layers.forEach((layer, index) => {
                ...
                const hoverClass = layer.hover_effect && layer.hover_effect !== 'none' ? `hsc-hover-${layer.hover_effect}` : '';
                const motionClass = layer.motion_effect && layer.motion_effect !== 'none' ? `hsc-motion-${layer.motion_effect}` : '';
                
                layersHTML += `
                    <div class="hsc-preview-layer ${isSelected ? 'selected' : ''} ${isLocked ? 'locked' : ''} ${hoverClass} ${motionClass}" 
        ...
        // RENDER PREVIEW PARTICLES
        function hscRenderPreviewParticles(effect) {
            const $container = $('#hsc-preview-particles');
            if (!$container.length || !effect || effect === 'none') return;
            ...
            if (effect === 'snow') {
                for (let i = 0; i < 30; i++) {
                    ...
                    html += `<div class="hsc-preview-particle hsc-preview-snow" style="left: ${left}%; width: ${size}px; height: ${size}px; animation-duration: ${duration}s; animation-delay: ${delay}s;"></div>`;
                }
                ...
                for (let i = 0; i < 15; i++) {
                    ...
                    html += `<div class="hsc-preview-particle hsc-preview-bubble" style="left: ${left}%; width: ${size}px; height: ${size}px; animation-duration: ${duration}s; animation-delay: ${delay}s;"></div>`;
                }
            }
            $container.html(html);
        }
        ...
        function hscGetPreviewLayerContent(layer) {
            if (layer.type === 'text') {
                ...
                const borderRadius = layer.border_radius ? `border-radius: ${layer.border_radius}px;` : '';
                return `<img src="${layer.image_url || ''}" style="max-width: 100%; display: block; pointer-events: none; ${borderRadius}" alt="">`;
            } else if (layer.type === 'button') {
